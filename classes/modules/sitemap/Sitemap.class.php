...
class PluginLsgallery_ModuleSitemap extends PluginLsgallery_Inherit_PluginSitemap_ModuleSitemap
{
    ...
    public function getExternalCounters()
    {
        ...
        $aCounters['albums'] = ceil(
            $this->PluginLsgallery_Album_GetCountAlbumsByFilter($aFilter) / Config::Get('plugin.sitemap.objects_per_page')
        );
        $aCounters['images'] = ceil(
            $this->PluginLsgallery_Image_GetCountImagesByFilter($aFilter) / Config::Get('plugin.sitemap.objects_per_page')
        );
     ...
     *
     * @param integer $iCurrPage Current Page
     *
     * @return array
     */
        ...
        if (false === ($aData = $this->Cache_Get($sCacheKey))) {
            ...
            foreach ($aAlbums['collection'] as $oAlbum) {
                $aData[] = $this->PluginSitemap_Sitemap_GetDataForSitemapRow(
                    ...
                    Config::Get('plugin.sitemap.lsgallery.albums.sitemap_priority'),
                    Config::Get('plugin.sitemap.lsgallery.albums.sitemap_changefreq')
                );
            }
            ...
            $this->Cache_Set(
                ...
                Config::Get('plugin.sitemap.lsgallery.albums.cache_lifetime')
            );
        }
    ...
    public function GetDataForImages($iCurrPage)
    {
        ...
        if (false === ($aData = $this->Cache_Get($sCacheKey))) {
            ...
            $this->Cache_Set(
                ...
                Config::Get('plugin.sitemap.lsgallery.images.cache_lifetime')
            );
        }
